...
function CreateCampaign({ onCancel, onCampaignCreated }) {
        ...
        return (
          <div>
            <div className="input-group">
              ...
              <textarea id="targetAudience" rows="4" value={formData.targetAudience} onChange={handleChange('targetAudience')} placeholder="e.g., 'Young professionals aged 25-40...'"></textarea>
            </div>
            <div className="input-group">
        ...
        return (
          <div>
            <div className="input-group">
              ...
              <textarea id="brief" rows="6" value={formData.brief} onChange={handleChange('brief')} placeholder="Describe the content you want influencers to create..."></textarea>
            </div>
  ...
  return (
    <div className="create-campaign-modal"><div className="create-campaign-form"><header className="wizard-header"><h2>Create New Campaign</h2><div className="step-indicator"><div className={`step ${step === 1 ? 'active' : ''}`}>Step 1: Basics</div><div className={`step ${step === 2 ? 'active' : ''}`}>Step 2: Audience</div><div className={`step ${step === 3 ? 'active' : ''}`}>Step 3: Creative</div></div></header><main className="wizard-content">{renderStepContent()}</main><footer className="wizard-actions">{step > 1 ? (<button className="wizard-button-prev" onClick={prevStep}>Back</button>) : (<div></div>)}{step < 3 && (<button className="wizard-button-next" onClick={nextStep}>Next</button>)}{step === 3 && (<button className="wizard-button-submit" onClick={handleSubmit}>Create Campaign</button>)}</footer></div></div>
  );
}
